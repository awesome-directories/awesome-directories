  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <section class="bg-gradient-to-br from-primary to-blue-600 text-white py-16 px-4">
      <div class="max-w-8xl mx-auto text-center">
        ...
        <p class="text-lg text-blue-100 mb-8">
          Stop wasting time on dead links and low-value directories. Get the 20 that actually drive traffic—in under 3 minutes.
        </p>
    ...
    <div class="bg-white border-b border-gray-200 sticky top-16 z-40 shadow-sm">
      <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        <!-- Active Filters Info -->
        <div v-if="activeFilterCount > 0 || searchQuery" class="mt-4 flex items-center justify-between">
          <div class="text-sm text-gray-600">
            Showing <span class="font-semibold">{{ filteredCount }}</span> directories
        ...
        <!-- Selection Actions -->
        <div v-if="selectedDirectories.length > 0" class="mt-4 flex items-center justify-between bg-blue-50 px-4 py-3 rounded-lg">
          <div class="text-sm text-gray-700">
            <span class="font-semibold">{{ selectedDirectories.length }}</span> director{{ selectedDirectories.length > 1 ? 'ies' : 'y' }} selected
          </div>
    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else-if="error" class="text-center py-12">
        ...
        <button @click="loadDirectories" class="mt-4 btn-primary">Try Again</button>
      </div>

      <!-- Empty State -->
      <div v-else-if="displayedDirectories.length === 0" class="text-center py-12">
        <div class="text-6xl mb-4">🔍</div>
        <h3 class="text-xl font-semibold text-gray-900 mb-2">No directories found</h3>
...
const toggleDirectorySelection = (directory) => {
  const index = selectedDirectories.value.findIndex(d => d.id === directory.id)
  if (index > -1) {
  ...
  localStorage.setItem('selectedDirectories', JSON.stringify(
    selectedDirectories.value.map(d => d.id)
  ))
}
...
const loadDirectories = async () => {
  ...
  if (storedSelection) {
    try {
      ...
      selectedDirectories.value = directories.value.filter(d => ids.includes(d.id))
    } catch (e) {
