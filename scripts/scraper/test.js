...
async function testUrl(browser, testSite) {
  ...
  try {
    ...
    console.log(`  - Description: ${homepageData.metadata.description?.slice(0, 100)}...`);
    console.log(`  - Hero: ${homepageData.hero.heading}`);
    console.log(`  - Features: ${homepageData.features.length} found`);
    console.log(`  - Pricing: ${homepageData.pricing.found ? 'Found' : 'Not found'}\n`);
    ...
    console.log(`  - Provides dofollow: ${linkData.quality.providesDofollow ? 'Yes ✓' : 'No ✗'}\n`);
    ...
    console.log(`  - Submission URLs found: ${smartCrawlData.submissionUrls.length}\n`);
    ...
    console.log(`  - Quality Score: ${compiledData.curationSuggestions.qualityScore}/100`);
    console.log(`  - Short Description: ${compiledData.curationSuggestions.shortDescription?.slice(0, 100)}...`);
    console.log(`  - Suggested Categories: ${compiledData.curationSuggestions.categories.join(', ')}`);
    console.log(`  - Link Quality: ${compiledData.curationSuggestions.linkQuality}`);
    console.log(`  - Key Features: ${compiledData.curationSuggestions.keyFeatures.length} found\n`);
...
async function runTests() {
  console.log('\n');
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║          Awesome Directories Web Scraper Test Suite           ║');
  console.log('╚════════════════════════════════════════════════════════════════╝');
  ...
  try {
    ...
    results.forEach(result => {
      if (result.success) {
        console.log(`✅ ${result.site} - Quality Score: ${result.qualityScore}/100`);
      } else {
